<template>
  <div id="page-sell-data" class="page-sell-data" @scroll="scrollPage">
...
export default {
    ...
    this.$comfun.http_post(this, `data/senior/consultant/${userId}`).then((response) => {
      if (response.body.success === '1') {
        ...
        if (this.userInfoData.user.photo !== null) {
          this.userHead = this.$moment.HttpAddress_1 + `showFile/${this.userInfoData.user.photo}`
        }
        this.resetDateEvery(this.dateTabs[0].id)

        this.userName = this.userInfoData.user.name
        setTimeout(() => {
          var startDate = this.$comfun.formatDate(this.userInfoData.basedate, 'yyyy-MM-dd')
  ...
  methods: {
    scrollPage () {
      var pageScrollTop = document.querySelector('#page-sell-data').scrollTop
    ...
    resetDateEvery (curDateType) {
      ...
      switch (curDateType) {
        case 'day':
          this.getDateSection(this.$comfun.formatDate(this.userInfoData.basedate, 'yyyy/MM/dd'), curDateType, 'M/d').then(() => {
        ...
        case 'week':
          ...
          break
        case 'month':
          this.getDateSection(this.$comfun.formatDate(this.userInfoData.basedate, 'yyyy/MM/dd'), curDateType, 'M').then(() => {
            this.loadHeaderAfterDateSection()
          })
          break
        case 'year':
          this.getDateSection(this.$comfun.formatDate(this.userInfoData.basedate, 'yyyy/MM/dd'), curDateType, 'yyyy').then(() => {
            this.loadHeaderAfterDateSection()
          })
          break
      }
    ...
    searchHeaderData (isChange) {
      ...
      this.$comfun.http_post(this, `data/public/contrast/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
        ...
        this.$comfun.http_post(this, `data/public/profit/${this.userInfoData.user.id}`, {
          startDate: startDate,
          endDate: endDate
        }).then((response) => {
          if (response.body.success === '1') {
          ...
          this.$comfun.http_post(this, `data/public/perf/${this.userInfoData.user.id}`, {
            startDate: startDate,
            endDate: endDate
          }).then((response) => {
    ...
    getLineChartData (type, startDate, endDate) {
      var chartLimitDate = ''
      // 整车
      this.$comfun.http_post(this, `data/public/newcar/curve/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
      ...
      // 二手车
      this.$comfun.http_post(this, `data/public/oldcar/curve/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
      ...
      // 保险
      this.$comfun.http_post(this, `data/public/insurance/curve/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
      ...
      // 汽车用品
      this.$comfun.http_post(this, `data/public/accessory/curve/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
      ...
      // 金融
      this.$comfun.http_post(this, `data/public/finance/curve/${this.userInfoData.user.id}`, {
        type: type,
        startDate: startDate,
        endDate: endDate
      }).then((response) => {
